...
unsigned char LCD_string_g[32]; //changed this to [32] from [16]
...
void LCD_Cursor(unsigned char column) {
	if ( column < 16 ) { //ieee lcd change to 16
		LCD_WriteCmdStart(0x80+column);
	}
	else {
		LCD_WriteCmdStart(0xB0+column); //ieee lcd change to 0xBF+ (for this time we will leave at 0xB0)
	}
}
...
int LCDI_SMTick(int state) {
	...
	case LI_PositionCursor:
		...
		break;
	case LI_DisplayChar:
		if (i<32 && LCD_write_str) { //changed value to 32 from 16
			state = LI_PositionCursor;
			LCD_WriteDataEnd();
			i++;
		}
